...
class NInputLogKunNoMD5(NInputFile):
    def __init__(self, datetime, product, item, tryTimes = 1, interval = 0, idx=-1, host="logdata.baidu.com"):
    ...
    def wait_for_log_ready(self,date_time,item):
        ...
        for i in range(self.tryTimes):
            cmd = "wget"
            site ="http://%s/?m=Data&a=GetData&token=%s"%(self.host,token)
    ...
    def wGetDownload(self, dateTime, product, item, oFile):
        cmd = "wget"
        site = "http://%s/?m=Data&a=GetData&token=ecom_nova_yd6sv6fjjnewg0dnc3me87" %(self.host)
